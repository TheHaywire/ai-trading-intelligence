"""Mean reversion strategy using VWAP bands and RSI extremes."""
...
class MeanReversionBandsStrategy(Strategy):
    """
    Mean reversion strategy for range-bound markets.

    Logic:
    1. Calculate rolling VWAP with ATR bands
    2. Entry: Price pierces k*ATR beyond VWAP + RSI(2) extreme
    3. Exit: Revert to VWAP, time stop 4h, hard SL at band+ATR
    4. Only trade during liquid sessions (Asia/London open)
    5. Block on red news
    6. Min hold 61s enforced by system

    Best for: Major FX pairs, Gold during tight spread sessions
    """

    def __init__(self, name: str = "MeanReversionBands", config: Optional[Dict] = None):
        """
        Initialize mean reversion strategy.

        Config parameters:
        - vwap_period: VWAP calculation period (default 20)
        - atr_period: ATR period (default 14)
        - band_multiplier: ATR band multiplier (default 2.0)
        - rsi_period: RSI period (default 2)
        - rsi_oversold: RSI oversold level (default 5)
        - rsi_overbought: RSI overbought level (default 95)
        - max_spread_points: Max spread in points (default 8)
        - time_stop_hours: Time-based stop (default 4)
        - allowed_symbols: List of symbols (default FX majors + gold)
        - allowed_sessions: List of sessions (default ["asia", "london"])
        """
        default_config = {
            "enabled": True,
            "vwap_period": 20,
    ...
    def analyze(self, state: MarketState) -> Optional[Signal]:
        ...
        # Session filter (simplified - would need actual session detection)
        current_hour_utc = state.timestamp.hour
        if not self._is_allowed_session(current_hour_utc):
            return None
        ...
        if current_price < lower_band and rsi < self.config["rsi_oversold"]:
            ...
            stop_loss = lower_band - (upper_band - lower_band) * 0.5  # Extra ATR cushion
            take_profit = vwap
    ...
    def _is_allowed_session(self, hour_utc: int) -> bool:
        ...
        if "asia" in allowed_sessions and 0 <= hour_utc < 8:
            return True
        if "london" in allowed_sessions and 7 <= hour_utc < 16:
            return True
        if "ny" in allowed_sessions and 13 <= hour_utc < 22:
            return True
