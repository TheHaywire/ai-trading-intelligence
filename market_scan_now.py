"""
IMMEDIATE MARKET SCAN - Check all opportunities NOW
Scans multiple asset classes and sends email with current opportunities
"""
...
# Expanded symbol list - Multiple asset classes
SYMBOLS = {
    'Metals': ['GOLD', 'SILVER', 'COPPER'],
    'Forex Majors': ['EURUSD', 'GBPUSD', 'USDJPY', 'USDCHF', 'AUDUSD', 'NZDUSD', 'USDCAD'],
    'Forex Crosses': ['EURJPY', 'GBPJPY', 'EURGBP', 'AUDNZD'],
...
def format_email(opportunities, all_data):
    ...
    message = f"COMPLETE MARKET SCAN - {datetime.now().strftime('%Y-%m-%d %H:%M')}\n\n"
    ...
    if opportunities:
        ...
        for opp in opportunities:
            message += f"{opp['symbol']} ({opp['category']})\n"
            message += f"  Price: ${opp['price']:.4f} | Score: {opp['score']}/10\n"
            message += f"  Trend: {opp['trend']} ({opp['trend_strength']:.2f}% separation)\n"
    ...
    for category, symbols in SYMBOLS.items():
        ...
        if category_data:
            ...
            for data in category_data:
                ...
                message += f"  {emoji} {data['symbol']}: ${data['price']:.4f} "
                message += f"({data['change_24h']:+.1f}% 24H) "
                message += f"[RSI: {data['rsi']:.0f}] "
                message += f"{data['trend']}\n"
            message += "\n"
    ...
    if valid_data:
        sorted_gainers = sorted(valid_data, key=lambda x: x['change_24h'], reverse=True)[:5]
        sorted_losers = sorted(valid_data, key=lambda x: x['change_24h'])[:5]

        message += "Biggest Gainers:\n"
        for i, data in enumerate(sorted_gainers, 1):
            message += f"  {i}. {data['symbol']}: {data['change_24h']:+.2f}% (${data['price']:.4f})\n"

        message += "\nBiggest Losers:\n"
        for i, data in enumerate(sorted_losers, 1):
            message += f"  {i}. {data['symbol']}: {data['change_24h']:+.2f}% (${data['price']:.4f})\n"

    message += f"\n\nScanned {len(valid_data)} instruments at {datetime.now().strftime('%H:%M:%S')}\n"
    message += "Next scan: Run manually or wait for hourly alert system\n"

    return message

# Initialize MT5
print("="*60)
print("COMPREHENSIVE MARKET SCAN STARTING")
print("="*60)
...
for category, symbol_list in SYMBOLS.items():
    ...
    for symbol in symbol_list:
        current += 1
        print(f"  [{current}/{total_symbols}] Checking {symbol}...", end=" ")
        result = check_symbol(symbol, category)
        if result:
            all_data.append(result)
            if result['signals']:
                print(f"[{result['score']} signals]")
            else:
                print("[OK]")
        else:
            print("[SKIP]")
